...
| `--json` | machine-readable output: one JSON document on stdout for `status`, `ls`, `config`, `doctor`, `check`, `switch`, `watch`, `rename`, `rm`, `uninstall` (`ok` mirrors the exit code, failures add `error`) |
