...
- Every periodic check tick samples the parked account whose last `/usage` attempt is oldest, skipping any attempted within `policy.usagePollTtlMs`, so no parked account is staler than one tick per pooled account and the swap trusts the cached figures.
