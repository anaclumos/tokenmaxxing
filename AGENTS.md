...
- `claude setup-token` was investigated and abandoned for the local pool: it is inference-only scoped and returns no rate-limit percentages, which breaks monitoring. Do not revisit without solving usage. The Cursor Cloud relay (1.26.0) uses setup tokens on the cloud VM only, where rotation is reactive on a limit error; that is not a precedent for the local path, and the local switching engine never reads one.
