...
- An ambient `CLAUDE_CONFIG_DIR` or `CLAUDE_SECURESTORAGE_CONFIG_DIR` is refused in CLI commands: on Linux the swap would write where the ambient var points while the child reads the default store, a silent wrong-account desync.
...
One rule (#81, 2026-09-12): under both bars the seat holds; at or over a bar, `pickBest` ranks the usable candidates by `pacePressure` (remaining weekly percent over time to the weekly reset, highest first, ties by soonest reset then lowest weekly use) and the seat swaps; with nothing usable, the depleted-wait path parks to the soonest reset within `policy.maxWaitMs`. Manual switching uses the same ranking. Policy lives in `docs/content/docs/switching.mdx`, with review lessons in `.cursor/rules/switch-verification.md`.

- One engine, two providers (#93): `decide.ts`, `picker.ts`, `status`, and the account commands take a `Provider` (`src/lib/provider.ts`) and never branch on its name. What differs per product lives in `src/lib/claude.ts` and `src/lib/codex.ts` (identity, credential store, usage source, presence, login and install flows, the swap) and in the codex entries (restart on swap, sibling reconcile). A codex-only rule is a provider capability (`waitsWhenDepleted`, `switchMargin`, `presentIds`, `gatedFamilies` returning `null` for "every named limit gates"), not an `if` in the engine. Every cached window carries its own duration and sample time, so `isSessionWindow` is one duration test for both providers.

- Build EVERY Claude PickCtx and trigger from `thresholdBars(cfg)`: `thresholds.session` and `thresholds.weekly`, one number each, minus `policy.projectionMargin`. Trigger and screening share the bars, or a swap lands on an account the trigger re-flags and ping-pongs on the cooldown beat. Codex reads the same bars. The session ladder, `hardThresholds`, the engagement floor, organization affinity, incumbent hysteresis, and the Layer 2 wall squeeze were deleted in #81; do not reintroduce a second bar per window.
...
- Selection reads cached figures only; the swap-time verification probe (its budget, kill deadline, abort plumbing, and the no-refresh probe mode) was deleted in #83 (2026-09-12). Freshness comes from the check tick instead: `sampleOldestParked` probes one parked account per tick, the one whose last attempt (`lastUsageAt` or `lastProbeAt`) is oldest, skipping any attempted within `policy.usagePollTtlMs`, and never inside the post-swap cooldown (the sampler refreshes an expiring parked token, and the just-parked seat's token is still in use during adoption). A failed attempt is recorded too, so a broken account does not starve the rest of the round-robin, and its cached figure stays as stale as it was. The pool lock covers only the reservation (`prepareParkedProbe`, which includes the credential refresh and identity checks) and the result write; the `/usage` child (`runParkedProbe`, up to three minutes) runs outside the lock so the StopFailure stamp and swap never wait on it. Do not move the child back under the lock. The tick's child gets one attempt (`retries: 0`, the round-robin is the retry loop) in its own `-tick` sample home so it never collides with a `status` probe of the same account; `finishParkedProbe` hands a rotated pair back to the parked slot only while the slot still holds the pair the child was given, and the result is stored only when it is newer than `lastUsageAt`.
...
- The periodic timer runs a plain `check` every `policy.checkIntervalMs` tick, the tick `init` writes into the timer unit; the self-paced cadence (`--if-due`, `nextcheck.json`, headroom bands) was deleted in #82 and is not to return.
...
- `/usage` on the live login is fail-silent whenever Claude's internal usage fetch errors or 429s, which is common while that token is busy serving sessions (2026-08-30 diagnosis: 10,198 failed probes), but it is not a dead path: on 2026-09-05 (2.1.261) every live probe of the day succeeded. So the statusLine tee stays the primary source for the active account, the decision engine falls back to the live probe when the tee is older than `usagePollTtlMs`, and `status` probes it only when the tee is missing or older than the stored sample. A silent result is unmeasured, never zero, and a sample never overwrites a newer one (`status` and the decision engine both keep the newest of tee and stored record). The tee carries no per-model rows, so the live Fable cap still comes from that probe and goes blind for hours under load; the StopFailure hook is the backstop, not a better probe.
...
- A Stop-hook text-sniffing limit failsafe. Stop stdin carries no `is_error`, so it fired on turns that merely discussed limits and stamped healthy accounts as walled. The real errored-turn signal is the StopFailure hook (`src/entries/stopfailurehook.ts`): it classifies only the transcript row Claude Code itself marked `isApiErrorMessage`, never prose, writes only `enforcedUntil` on the account, inside the decision's own critical section so the stamp and the candidate selection cannot interleave with another hook's stamp (no usage-window rewrite, no per-model stamp, no second enforcement field; #90 and #118, 2026-09-12), and ignores a failure inside the post-swap cooldown because a request begun on the old credential can end on the new seat. That cooldown test is ordered by the swap clock, not by account equality: the hook captures the clock and the live account in one pool-lock section at entry, and a clock that has changed by the time stdin is read means the swap landed after the capture, so the failure still walls the captured account. Keep it that way.
