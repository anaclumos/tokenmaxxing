...
- State files that exist but fail to parse THROW. A truncated `accounts.json` read as an empty pool once let `init` overwrite it. Every loader goes through `readJson` in `src/lib/json.ts`; `nextcheck.json` is the one documented lenient file (`tryReadJson`, corrupt means due now). Statusline and hook stdin fields keep `.catch(undefined)` because a per-turn renderer and the StopFailure backstop must survive payload drift; that is the only place `.catch` belongs.
