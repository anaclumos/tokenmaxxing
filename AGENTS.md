...
- State files that exist but fail to parse THROW. A truncated `accounts.json` read as an empty pool once let `init` overwrite it. Every loader goes through `readJson` in `src/lib/json.ts`, the respawn markers included: a corrupt respawn marker stops the supervisor with the path instead of resuming a codex session on stale in-memory credentials. Two owned files are lenient by design: `nextcheck.json` (`tryReadJson`, corrupt means due now) and the codex reconcile marker (the Stop hook deletes a corrupt one and logs the parse error, because `reconcileNonLiveSiblings` skips a sibling whose marker file exists and re-signals it on the same turn once the file is gone). The onboard poll of claude's own half-written `.claude.json` is a vendor file mid-login, not state. Statusline and hook stdin fields keep `.catch(undefined)` because a per-turn renderer and the StopFailure backstop must survive payload drift; that is the only place `.catch` belongs.
