...
Accounts rank same-organization first, then by pace pressure (remaining percent over time to weekly reset, highest first), never by most-remaining. Mechanics live in `src/lib/decide.ts` and `src/lib/picker.ts`; rationale and policy in `docs/switching.mdx`, `.memory/switch-policy-pace-pressure.md`, and `.memory/stopfailure-enforced-limit-signal.md`. The vocabulary below is used across both files.
...
- The prompt cache is organization-scoped, so a cross-org swap makes every hot session re-upload its context at the cache-write rate (profiled 2026-09-10, `.memory/swap-cost-profile.md`: 8.7% of weighted input over 13 days, up to 18.5% of one 5-hour window). `swapPreference` therefore ranks the seat's own organization first and pace pressure within the tier, and the greedy path never crosses orgs while the seat is usable. Every automatic swap first verifies a candidate whose sample is older than `usagePollTtlMs` with one free `probeParkedUsage` and skips it for the evaluation when it is really over a bar (`decide.candidate_walled`); a failed probe leaves the cached figures in force (`decide.candidate_unverified`). Do not reintroduce a swap that trusts a stale parked sample.
