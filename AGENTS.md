...
- State files that exist but fail to parse THROW. A truncated `accounts.json` read as an empty pool once let `init` overwrite it. Every loader goes through `readJson` in `src/lib/json.ts`, the respawn and reconcile markers included: a corrupt marker stops the supervisor with the path instead of resuming a codex session on stale in-memory credentials. `nextcheck.json` is the one documented lenient state file (`tryReadJson`, corrupt means due now); the onboard poll of claude's own half-written `.claude.json` is a vendor file mid-login, not state. Statusline and hook stdin fields keep `.catch(undefined)` because a per-turn renderer and the StopFailure backstop must survive payload drift; that is the only place `.catch` belongs.
