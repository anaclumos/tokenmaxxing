...
- Core deps are zod, es-toolkit, ky, and proper-lockfile (the pool and refresh locks). The global default stack does not apply (there is no date-fns here, date math goes through `Intl` in `parseResetClock`).
