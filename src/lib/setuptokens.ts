...
const SetupTokenSchema = z.object({ accountUuid: z.string().min(1), label: z.string().min(1), token: z.string().min(1), mintedAt: z.number() });
...
export function cursorSecretValue(tokens: CloudToken[]): string {
  return JSON.stringify(CloudTokensSchema.parse(tokens));
}
