...
- **GREEDY path**: engaged but under every bar. Rank all accounts same-organization first, then by pace pressure; keep seat on best-or-tie (`currentWins`); else swap to strictly better. The organization tier means the greedy path never crosses organizations while the seat is usable. Never depleted-waits or pre-parks. The automatic path adds incumbent hysteresis (`policy.greedySwapMargin`, default 0.15): hold the seat unless a parked account's pace pressure exceeds the current one's by more than that fraction, so near-tied accounts stop trading the seat and busting the per-org prompt cache. The margin gates lateral under-bar swaps only, never a crossed bar or the wall; bare `tokenmaxxing switch` uses margin 0 and always takes the strictly best account.
- **HARD path**: a screening bar crossed. Swap to the best usable target, same-organization seats first; if none, Layer 2 wall logic (Claude only). Before any automatic swap, a candidate whose sample is older than `policy.usagePollTtlMs` is probed live once (free `/usage` on its parked credential) and skipped for the evaluation when it is really over a bar; a probe with no data leaves the cached figures in force.
- **Organization tier**: the prompt cache is organization-scoped, so a cross-organization swap makes every hot session re-upload its context at the cache-write rate. A usable candidate in the seat's own organization always outranks one outside it; pace pressure orders within the tier.
