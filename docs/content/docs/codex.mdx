...
- Codex banks one-time usage-limit resets on the account (promotional or referral grants that expire); consuming one refreshes the 5-hour and weekly windows together and moves the weekly reset date. With `codex` in `policy.preferToUseBankedReset`, a seat over the top rung rides to its wall while the usage endpoint reports a credit, then consumes one and keeps the session running, no restart: see [Banked resets](/switching#banked-resets). tokenmaxxing never buys a paid instant reset.
