...
Below `policy.greedySessionFloor` (default 50, in percent of the 5-hour session window), the decision does not run at all - a fresh session rides its account. Once the active session window is 50% used, or any bar is crossed, the decision engages.

## Screening bars

The thresholds are screening bars, not the trigger for panic:

- `thresholds.session`: a ladder, default `[50, 80, 95]`. Each evaluation resolves one active rung: the lowest rung at which some pooled account, the current one included, is still usable. While any account sits under 50, the bar is 50, so a seat that crosses 50 hands off to that account. Once every account is past 50 the bar climbs to 80, and past 80 everywhere it climbs to 95. The top rung is the terminal bar; beyond it the wall (below) takes over. A 5-hour reset is cheap to sit out, so the pool drains level by level and no account runs far ahead of its siblings.
