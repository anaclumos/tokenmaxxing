...
This keeps the last few percent of each account's quota from going unspent. The wall reading comes from whatever usage source the decision already uses - the statusline tee on a supervised interactive session, or a `/usage` probe on a headless run - the same figures Claude's `/rate-limit-options` shows, so an account that genuinely maxes out is detected and skipped. A single-turn overshoot is caught the next time the decision runs - the periodic check, or the next spawn or Stop boundary on a headless run - and `serve` additionally persists an errored limit through the observed-limit failsafe so its retry sees the walled account at once.
