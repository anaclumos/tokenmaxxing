...
| `setup-tokens.json` | Cursor Cloud only: `{label, token, mintedAt}` per pooled account, mode 0600, the source of the `TOKENMAXXING_TOKENS` secret value that `setup-token` prints |
