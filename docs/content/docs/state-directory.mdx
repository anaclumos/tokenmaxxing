...
| `relay.json` | Durable tmux relay config (default worker, `defaultPermissionMode: auto`, timeouts, `idleTtlMs`, host model hints, `sessionPrefix`) |
| `relay/` | Relay companion state: `sessions/<uuid>.json` registry, per-session `locks/`, additive `turn-done/<uuid>` markers (never under `respawn/`), `pending/` permission pings, `decisions/` approve/deny answers |
