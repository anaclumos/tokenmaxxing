...
| `lock.lock`, `lock.lock.pid` | The pool lock directory (proper-lockfile, 60-second stale bar) that serializes swaps, and the holder's pid beside it so a paused holder is not reclaimed as dead |
