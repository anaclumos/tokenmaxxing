...
| `usage.json` | The live account's usage snapshot: the statusline tee's aggregate windows plus the per-model weekly caps the `/usage` probe adds as rows on the same account; its mtime is the feed's liveness heartbeat |
