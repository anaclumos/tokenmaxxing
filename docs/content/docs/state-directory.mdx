...
| `lock.lock` | The pool lock directory (proper-lockfile, 60-second stale bar) that serializes swaps |
...
| `codex-*` | The parallel Codex pool: `codex-accounts.json`, `codex-lastswap.json`, `codex-lock.lock`, `codex-creds/`, `codex-onboard/`, `codex-respawn/`, `codex-reconcile/` (cross-session signals asking a sibling session's supervisor to respawn onto the live account), and `codex-live/` (one PID-validated presence file per running supervised codex session, so a live account is never a swap target) |
