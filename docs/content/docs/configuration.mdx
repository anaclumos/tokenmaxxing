...
- `hardThresholds.session` / `hardThresholds.weekly`: the wall bars (default 100, each account's real server limit). When every account is over its screening bar, the automatic decision squeezes each one up to its wall before parking - see [The wall](/switching). Set equal to `thresholds` to disable.
