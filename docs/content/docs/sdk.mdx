...
The package's programmatic entry (`exports["."]` points at `src/sdk.ts`) pairs tokenmaxxing with the Claude Agent SDK for agents you build against your own pooled accounts. Your app must run under Bun (tokenmaxxing ships TypeScript source with Bun-only APIs).
