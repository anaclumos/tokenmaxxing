...
One switching engine serves both pools. The account model, the picker, the decision, and the account commands take a provider (`src/lib/provider.ts`): the Claude provider (`src/lib/claude.ts`) wires the credential store, the OAuth profile and refresh endpoints, the statusline tee, and the `/usage` probe; the Codex provider (`src/lib/codex.ts`) wires `auth.json`, its id token, the usage GET, presence, and restart on swap (see [Codex support](/codex)). Everything below describes the Claude provider unless it says otherwise.
