...
2. Installs the target account's freshly refreshed credential as the live one, merging into the existing live blob so sibling state (such as per-MCP-server OAuth tokens) survives. (The target's rotated refresh token was already persisted to its own backup the instant the refresh returned, before the lock was taken.)
3. Atomically rewrites the `oauthAccount` field in `~/.claude.json`.
4. Commits the new active label and clears stale usage snapshots before the lock releases. These separate files cannot be crash-atomic together; the divergence window is a single write, and the next swap resolves the live credential's true owner independently of the label, detecting and logging any crash drift.
