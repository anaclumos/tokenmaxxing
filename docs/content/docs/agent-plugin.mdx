...
| `pool_status` | `tokenmaxxing status` (never `--ping`) |
...
**Not exposed:** `status --ping`, credential dumps, `init` / `add` / `auth` / `rm` / `uninstall`, process kills.
...
| `switching-policy` | One bar per window, pace pressure, model-family matching |
| `sdk-pairing` | Agent SDK helpers |
| `codex-pool` | Restart-is-switch, no depleted pause |
