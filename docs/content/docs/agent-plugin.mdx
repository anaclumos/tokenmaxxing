...
tokenmaxxing ships a portable [Agent Plugins 1.0](https://agent-plugins.org/) package at `agent-plugin/`. Compatible clients load that directory as a local plugin. Skills under `skills/`. MCP from `mcp.json`. Cursor, Codex, ChatGPT, VS Code, Copilot, and friends.
...
| `relay-session` | Durable tmux relay via `tokenmaxxing relay`; Claude permission modes, Cursor Run Mode auto-approve rule, Codex mapping, IPC matrices |
...
Thin shell-only agents under `agent-plugin/agents/`:

- `tokenmaxxing-claude.md` — Claude Code worker through `relay turn --worker claude`
- `tokenmaxxing-codex.md` — Codex worker through `relay turn --worker codex`

Install into host agent dirs with `tokenmaxxing relay install --target cursor|claude|all` (merges only tokenmaxxing-owned hook keys). Host finish hooks must fire on every return, including `permission-needed` pings.
