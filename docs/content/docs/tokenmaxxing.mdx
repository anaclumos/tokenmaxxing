...
The package's `exports` map exposes `src/sdk.ts` as the programmatic entry described above. Releases are tagged `v<version>` on GitHub; CI publishes to npm via trusted publishing (OIDC, with provenance), and the publish gate runs the typecheck and the full test suite first.
