...
Update 2026-08-30: the family-token matching was intact; the live Fable cap goes blind because the live-token `/usage` probe is fail-silent under session load, and the enforcement is caught by the StopFailure hook instead. See [[stopfailure-enforced-limit-signal]].
