...
Goal: ship 1.13.1 so the active account's status row can never silently show a stale tee or regress the stored sample, and so a refused --ping ping says why.

- [x] Diagnose: plain `status` showed a 20-minute-old tee at 21% while `--ping` probed live at 100%; the tee had frozen when the host's hooks and timer were stripped; the plain run then overwrote the fresh record with the older tee (adversarially verified by three read-only opus lenses)
- [x] `status.ts`: tee adopted only when its `ts` is not older than the account's `lastUsageAt` (else live probe, or under --ping the failed probe stands); tee-sourced rows print `statusline tee <age>` with `(stale)` past `usagePollTtlMs`; `pingRejected` in the report; `ping rejected at the <window> limit: <text>` in text mode
...
- [x] Hermetic runs under throwaway roots: tee newer than record (adopted, age line), tee older (ignored, probe attempted, record kept), `check` with tee older/newer (record kept/updated), `--ping` against a fake claude replaying the real rejection row (`ping rejected at the 5h limit`); `pingRejection` replayed on the real failed-ping transcript
