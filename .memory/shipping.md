...
- Poll over REST, never GraphQL: `gh api repos/<owner>/<repo>/pulls/<n>`, `.../pulls/<n>/comments`, `.../issues/<n>/comments`, `.../commits/<sha>/check-runs`, `.../commits/<sha>/status`, and `.../issues/<n>` for the source issue. `gh api graphql`, `gh pr view --json`, `gh pr checks`, and `gh pr merge` all spend the per-user GraphQL quota, which the owner's other sessions share, and `gh api rate_limit` does not show the exhaustion. Merge over REST too: `gh api -X PUT .../pulls/<n>/merge` with `merge_method` `squash` and the head `sha`, so a moved head cannot merge. Keep one GraphQL read for the final review-thread check.
...
- The docs site has 30 locales. The ko, ja, zh, yue, and wuu pages are hand-humanized: edit them in place and never regenerate them from English. Quote every frontmatter value that contains a colon. A localized sync can follow in its own PR; it is not a merge blocker for the English change.
