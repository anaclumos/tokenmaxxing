...
- SDK facts verified 2026-07-18 (0.3.214 d.ts + code.claude.com docs): omitted `systemPrompt` = MINIMAL prompt (NOT the claude_code preset - serve has always run minimal); omitted `settingSources` = all sources (CLI parity), so CLAUDE.md loads; skills work outside the system prompt (omitted `skills` option = discovered skills enabled, Skill tool on); plugin skills namespace as `<plugin>:<skill>`, manifest `.claude-plugin/plugin.json` optional; `UserPromptSubmit` hook returns `{hookSpecificOutput: {hookEventName, additionalContext}}`.
