...
- NO FORCE PATH - repo policy (hook-enforced 2026-07-18): the bot must NEVER discard work. The gate is a residue check, `git status --porcelain --ignored` in the worktree: ANY modified, untracked, or IGNORED file refuses with the offenders listed (stricter than `git worktree remove`'s own check, which silently deletes ignored .env/node_modules - review-verified 2026-07-18 that git's gate is porcelain-status based and excludes ignored files). A first draft had `force: true` on the tool and it was stopped by the repo hook; do not reintroduce it.
- Unmerged branch = ARCHIVED, never left canonical (second hook catch 2026-07-18): `branch -d`'s "not fully merged" refusal renames the branch to `tm-slack-<key>-kept-<shortsha>` (`branch -m`), because a kept canonical branch makes ensureThreadCwd's crash-recovery reattach path silently resurrect the stale tip into a session the user was told is fresh. The revival regression is pinned in test/slackbridge.test.ts (fresh mention after archive = worktree at repo HEAD).
- A refusal keeps the record AND the subscription so the thread stays live for a retry after committing.
- Cleanup is keyed ENTIRELY on the thread record ({cwd, repo}), never the current link config (adversarial review catch 2026-07-18): re-linking a channel `--no-worktree` or onto another repo must still collect its older worktree threads; `cleanupThread` takes no link at all.
