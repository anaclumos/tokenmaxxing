...
- Mention plumbing (adapter 4.34.0 source-verified): OUTGOING streamed `markdown_text` deltas pass raw (no escaping); the post-and-edit fallback `finalize` runs `linkBareMentionNames` (bare `@U...` -> `<@U...>`) and leaves formed `<@U...>` intact. Slack's own docs never state whether `markdown_text` renders `<@U...>` as a notifying mention - live-verify on first real use. INCOMING mrkdwn is normalized to markdown (`<@U...>` -> bare `@U...`), so `stripLeadingMention`'s `<@` check no longer strips the leading bot mention (observed live: a relayed prompt began "@U0BHS1YKNSK add related skills"); known drift, surfaced to the user, NOT fixed (out of scope).
