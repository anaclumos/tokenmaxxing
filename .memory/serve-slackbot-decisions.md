...
- Todo display (user ask 2026-07-18 "display todo item in a clever way"): TodoWrite maps to ONE stable-id "Todos" checklist card per stream (`todos`, subagents `todos-<parent>`), updated in place per call: ✅ content / 🔄 activeForm (in-progress shows live narration) / ⬜ content, card complete only when all items are completed. No generic card, no segment break (bookkeeping must not split messages), success tool_result suppressed, FAILED TodoWrite flips the card to error (unmeasured must not look safe). Title stays constant "Todos": same-id re-append updating status/details is live-verified, title mutation is not. Card ids do not carry across segments (accepted: fresh card in the newest message). GOTCHA (PR #5 review catch, verified 2026-07-18: SDK 0.3.214 + claude 2.1.214 + official todo-tracking docs): claude >= 2.1.142 defaults to TaskCreate/TaskUpdate and never emits TodoWrite, so relayThread sets CLAUDE_CODE_ENABLE_TASKS=0 in the spawn env; without the opt-out the card is inert. Empty todos [] maps to null on purpose (claude clears only after all-completed; keep the finished card visible).
