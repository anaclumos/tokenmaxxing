...
- **Thread = session**: a bot mention in a linked channel subscribes the thread, creates `slack-worktrees/<threadKey>` (branch `tm-slack-<threadKey>` cut from the repo's HEAD; `--no-worktree` links run in the repo itself), and records `{threadId, cwd, sessionId}` under `slack-threads/`. Resume is cwd-keyed in claude, so the cwd stays byte-stable for the thread's life; worktrees are deleted only through the finish flow below (they hold the thread's work).
- **Thread GC (finish_thread)**: when the user says the work is finished, the model calls the in-process MCP tool `finish_thread` (a per-turn `createSdkMcpServer` in relayThread, `alwaysLoad: true`, granted via `allowedTools` since nobody can answer a permission prompt over Slack). The handler runs in the daemon but only flags the turn outcome; after the turn ends (claude subprocess gone, segments posted) serve.ts runs `cleanupThread`: a residue check (ANY modified, untracked, or ignored file refuses with the offenders listed - stricter than git's own gate, which would silently delete ignored .env/scratch files; the bot never discards work, there is deliberately no force path), then `git worktree remove`, `git branch -d` (an unmerged branch is archived as `tm-slack-<key>-kept-<shortsha>` so a revived thread cuts fresh from HEAD instead of reattaching the stale tip), delete the `slack-threads/` record, `thread.unsubscribe()`, and post one confirmation line. A hand-deleted worktree dir is `git worktree prune`d first. Cleanup is keyed entirely on the thread record (cwd + repo), never the current link config, so a re-linked channel still collects its older worktree threads. A refusal keeps the record and subscription so the thread stays live for a retry after committing; after a successful GC a fresh mention starts a new session.
