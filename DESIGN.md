...
- **Thread GC (finish_thread)**: when the user says the work is finished, the model calls the in-process MCP tool `finish_thread` (a per-turn `createSdkMcpServer` in relayThread, `alwaysLoad: true`, granted via `allowedTools` since nobody can answer a permission prompt over Slack). The handler runs in the daemon but only flags the turn outcome; after the turn ends (claude subprocess gone, segments posted) serve.ts runs `cleanupThread`: `git worktree remove` (plain - a dirty worktree ALWAYS refuses; the bot never discards uncommitted work, there is deliberately no force path), `git branch -d` (an unmerged branch is kept), delete the `slack-threads/` record, `thread.unsubscribe()`, and post one confirmation line. A hand-deleted worktree dir is `git worktree prune`d first. A refusal keeps the record and subscription so the thread stays live for a retry after committing; after a successful GC a fresh mention starts a new session.
