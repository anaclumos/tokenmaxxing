...
export const piAgent: ClientParser = {
  ...
  async *parse(): AsyncGenerator<UsageRecord> {
    for await (const file of glob(join(PI_DIR, "**", "*.jsonl"))) {
      ...
      let model = "unknown", lastTs = "", sid = "", project: string | undefined;
      for await (const e of readJsonl<PiEntry>(file)) {
        if (e.type === "session") {
          if (e.id) sid = e.id;
          if (e.cwd) project = projectFromCwd(e.cwd);
          continue;
        }
      ...
      yield { client: "pi-agent", model, sessionHash: sessionHash("pi-agent", sid || file), timestamp: lastTs || new Date().toISOString(), tokens: { input, output, cacheRead, cacheWrite, reasoning: 0 }, costUsd: 0, project };
    }
  },
};
