...
type Token = { id: string; prefix: string; name: string; lastUsedAt: string | null; createdAt: string };
...
export function TokenManager() {
  ...
  useEffect(() => {
    fetch("/api/tokens").then((r) => r.json()).then((d: { tokens: Token[] }) => setTokens(d.tokens));
  }, []);
    ...
    <div>
      {newToken && (
        <div className="mb-4 rounded border border-green-800 bg-green-950 px-4 py-3">
          <p className="mb-1 text-sm font-medium text-green-400">Token created. Copy it now - it won't be shown again.</p>
          <code className="block break-all font-mono text-sm text-green-300">{newToken}</code>
          <p className="mt-2 text-xs text-muted-foreground">
            Run: <code className="font-mono">tokenmaxxing login --token {newToken}</code>
          </p>
        </div>
      )}
